...
def load_processed_data():
    ...
    df_long_all = pd.read_sql_query("SELECT * FROM summary_frequencies", conn)
    stats_df = pd.read_sql_query("SELECT * FROM stats_results", conn)
    df_all = pd.read_sql_query("SELECT * FROM df_processed_wide", conn)

    conn.close()
    return df_long_all, stats_df, df_all

df_long_all, stats_df, df_all = load_processed_data()
...
with tab2:
    st.header("Statistical Comparison (Melanoma, Miraclib, PBMC)")
    ...
    p3_mask = (
        ...
        (df_long_all["response"].str.lower().isin(["yes", "no"]))
    )
    df_part3 = df_long_all[p3_mask]
...
with tab3:
    ...
    with c1:
        ...
        st.dataframe(baseline_df["project"].value_counts().reset_index(name="Sample Count"))
    with c2:
        st.write("Responders vs Non-Responders")
        st.dataframe(unique_subjects["response"].value_counts().reset_index(name="Subject Count"))
    with c3:
        st.write("Males vs Females")
        st.dataframe(unique_subjects["sex"].value_counts().reset_index(name="Subject Count"))
